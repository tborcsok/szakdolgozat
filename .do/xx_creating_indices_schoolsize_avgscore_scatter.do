...
*sort freq_omid
*by freq_omid: drop if freq_omid <=20
*drop freq_omid
...
*outreg2 e_1 using ${stataOUT}tables/elso_`w'.tex", replace ctitle(E1) `1keep' addtext(RE, NEM, FE, NEM) tex(fragment pretty)
...
*outreg2 e_2 using ${stataOUT}tables/elso_`w'.tex", append ctitle(E2) `2keep' addtext(RE, NEM, FE, NEM) tex(fragment pretty)
drop E2_source
...
*outreg2 i_1_temp using ${stataOUT}tables/elso_`w'.tex", append ctitle(I1) `1keep' addtext(RE, IGEN, FE, NEM) tex(fragment pretty)
...
*outreg2 i_2_temp using ${stataOUT}tables/elso_`w'.tex", append ctitle(I2) `2keep' addtext(RE, IGEN, FE, NEM) tex(fragment pretty)
...
*outreg2 f_1 using ${stataOUT}tables/elso_`w'.tex", append ctitle(F1) `1keep' addtext(RE, NEM, FE, IGEN) tex(fragment pretty)
...
*outreg2 f_2 using ${stataOUT}tables/elso_`w'.tex", append ctitle(F2) `2keep' addtext(RE, NEM, FE, IGEN) tex(fragment pretty)
...
drop if I1 ==.
bysort omid: gen school_size = _N
